...
class Gaussian(object):
    def __init__(self, jsonfiles=list_of_files):
        ...
        for f in jsonfiles:
            ...
            try:
                olts = ontjson['olt']
                self.oltjson_list.append(ontjson)
            except:
                continue
        assert len(self.oltjson_list) > 0
        lst_fn = self.jsonfiles[-1]
        lst_olts = self.oltjson_list[-1]

        # get the last datetime onts
        self.onts = []  # current onts
        for _olt in olts:
            _onts = _olt.get('ont')
            if len(_onts) > 0:
                _onts = [x for x in _onts if x.get('status') == 'online']
            self.onts += _onts
        # TODO: check the documentation to see if there's anything wrong
        # get the rx list from currently online onts
        self.rx_list = [x.get('rx') for x in self.onts if x.get('status') == 'online']
        print(self.onts)

    def get_gs_last(self):
        pass
